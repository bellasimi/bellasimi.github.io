...
> Temporay Dead Zone : 선언문을 만나기까지 일시적으로 접근할 수 없는 상태
...
은닉화  - 함수에 변수를 숨겨서 보호할 수 있음 

모듈화 - 클로저 함수를 각각의 변수에 할당해서 사용

캡슐화 - 클로저 모듈 패턴을 사용해 객체에 담아 여러개의 함수를 리턴할 수 있다.
...
setTImeout 종료 후 콜백함수가 실행 →그당시의 i는 2만 존재 →2+2 = 4
